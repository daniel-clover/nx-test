...
class ForkedProcessTaskRunner {
    ...
    forkProcessForBatch({ executorName, taskGraph: batchTaskGraph }, fullTaskGraph) {
    ...
    forkProcessPipeOutputCapture(task, { streamOutput, temporaryOutputPath, taskGraph, }) {
        return new Promise((res, rej) => {
            try {
    ...
    forkProcessDirectOutputCapture(task, { streamOutput, temporaryOutputPath, taskGraph, }) {
        return new Promise((res, rej) => {
            try {
                const args = (0, utils_1.getPrintableCommandArgsForTask)(task);
                if (streamOutput) {
    ...
    getEnvVariablesForProcess() {
        return Object.assign(Object.assign({}, process.env), this.getNxEnvVariablesForForkedProcess(process.env.FORCE_COLOR === undefined ? 'true' : process.env.FORCE_COLOR));
    }
    getEnvVariablesForTask(task, forceColor, outputPath, streamOutput) {
        ...
        const taskEnv = this.unloadDotEnvFiles(Object.assign({}, process.env));
        const res = Object.assign(Object.assign({}, (process.env.NX_LOAD_DOT_ENV_FILES === 'true'
            ? this.loadDotEnvFilesForTask(task, taskEnv)
            : // If not loading dot env files, ensure env vars created by system are still loaded
                taskEnv)), this.getNxEnvVariablesForTask(task, forceColor, outputPath, streamOutput));
    ...
    loadDotEnvFilesForTask(task, environmentVariables) {
    ...
    unloadDotEnvFiles(environmentVariables) {
